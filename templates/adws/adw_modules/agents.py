...
from . import agent_codex, agent_pi, permissions, prompts
from .data_types import (AgentCall, AgentConfig, EnvelopeBase, EventRecord,
                         GateCheck, GateReport, Phase, PiRequest, CodexRequest,
                         FlowieConfig, UsageBreakdown)
...
def load_config(path: str = "adws/flowie_config/flowie.config.yaml") -> FlowieConfig:
    ...
    return FlowieConfig(**raw)


def resolve(cfg: FlowieConfig, name: str) -> AgentConfig:
...
def validate(cfg: FlowieConfig, required: list[str]) -> None:
    ...
    for name in required:
        try:
            ...
            continue
        if agent.coding_agent not in ("codex", "pi"):
            problems.append(f"agent {name!r}: coding_agent {agent.coding_agent!r} "
                            f"is not implemented in this port (codex or legacy pi only)")
        ...
        try:
            if agent.coding_agent == "pi":
                ...
                agent_codex.validate_model(agent.model)
...
def execute(run, phase: Phase, call: AgentCall) -> EnvelopeBase:
    """One agent call: render prompts -> coding agent -> typed parse -> gates -> envelope."""
    ...
    def send(prompt_text: str):
        nonlocal latest, current_session_id
        if agent.coding_agent == "pi":
            request = PiRequest(
                prompt=prompt_text,
                ...
                # absolute: these are read by the subprocess, which runs in repo_root
            ...
            runner = agent_pi.run
            event_forwarder = _pi_event_forwarder(run, phase, agent.name)
        else:
            request = CodexRequest(
                ...
                model=agent.model,
                thinking=agent.thinking,
                session_id=current_session_id if not current_session_id.startswith("flowie-") else "",
                raw_output_path=str((agent_dir / "raw_output.jsonl").resolve()),
                last_message_path=str((agent_dir / "last_message.txt").resolve()),
                output_schema_path=str(schema_path.resolve()),
                cwd=str(run.repo_root),
            )
            runner = agent_codex.run
            event_forwarder = _codex_event_forwarder(run, phase, agent.name)
        result = runner(
            request,
            on_event=event_forwarder,
        ...
        if getattr(result, "session_id", "") and result.session_id != current_session_id:
            current_session_id = result.session_id
            run.save_agent_map(agent.name, {"session_id": result.session_id, "model": agent.model,
                                            "coding_agent": agent.coding_agent})
...
def _agent_session_id(run, agent: AgentConfig) -> str:
    ...
    return f"flowie-{run.adw_id}-{agent.name}-{new_id(4)}"


def _pi_event_forwarder(run, phase: Phase, agent_name: str):
...
def _codex_event_forwarder(run, phase: Phase, agent_name: str):
