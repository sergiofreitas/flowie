...
The user prompt asks for the shape; the type enforces it. They always travel as a pair, which is what lets one agent serve many calls — same system prompt, different user prompt + output type per call site. Output types live in code, never in `flowie.config.yaml`.
...
In the Codex port, `agent_codex.run()` uses `codex exec --output-schema` for the first send and `codex exec resume <session_id>` for corrections when the Codex JSONL stream exposes a resumable session id. Before parsing, the harness still tolerates a fenced `json` code block or prose wrapped around the object, but the prompt asks for bare JSON and every failed attempt is persisted as an invalid envelope row.
...
└── {agent_name}/
    ...
    ├── last_message.txt    final Codex assistant message for this send
...
```json
{
  "planner": {"session_id": "019f...",
              "model": "gpt-5.5", "coding_agent": "codex"},
  "builder": {"session_id": "019f...",
              "model": "gpt-5.5", "coding_agent": "codex"}
}
```
...
The map records the model each session was created with. If config drift changes an agent's model, that agent starts a **fresh** session and the map is updated — never a bad resume. `agent_sessions` in `flowie.db` is the queryable mirror of this file.

**Files are the raw record; the db is the queryable mirror.** Losing `flowie.db` loses nothing that can't be rebuilt from `raw_output.jsonl`, `envelope.json`, and `agent_map.json`.
