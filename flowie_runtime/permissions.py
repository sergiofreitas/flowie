...
Two keys drive it, both in flowie.config.yaml:
...
from .data_types import AgentConfig, FlowieConfig
...
def _glob(pattern: str) -> re.Pattern:
    ...
    `.flowie/adws/*.py` would match `.flowie/data/sessions/x/y.py` as well as the
...
def always_writable(cfg: FlowieConfig) -> list[str]:
...
def permitted(path: str, agent: AgentConfig, cfg: FlowieConfig) -> bool:
