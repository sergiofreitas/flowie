...
concrete type in flowie_runtime/data_types.py and swap it in).
"""
...
Usage:
    flowie run {name} "<prompt or path/to/prompt.md>" [--config .flowie/flowie.config.yaml] [--adw-id a1b2c3d4]
...
from flowie_runtime.data_types import AgentCall, PhaseParams, {imports}
...
def main() -> int:
    ...
    parser.add_argument("--dest-dir", default=".flowie/adws",
                        help="directory for generated ADWs")
    ...
    dest = Path.cwd() / args.dest_dir / f"{args.name}.py"
    ...
    print(f"run: flowie run {args.name} \"your request\"")
    return 0
