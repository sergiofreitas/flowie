...
Usage:
    uv run adws/adw_build_test.py "<prompt or path/to/prompt.md>" [--config .flowie/flowie.config.yaml] [--adw-id a1b2c3d4]

Phases: engineer(request) -> builder -> code(test) [-> builder(fix) -> code(test) ... bounded]

Testing is CODE. The suite's command is written down in .flowie/quality.py,
...
from flowie_runtime import agents, gates, quality, session, utils
from flowie_runtime.data_types import AgentCall, BuildOutput, PhaseParams
