...
Usage:
    uv run adws/adw_simple_sdlc.py "<prompt or path/to/prompt.md>" [--config .flowie/flowie.config.yaml] [--adw-id a1b2c3d4]
...
from flowie_runtime import agents, changes, gates, git_helper, quality, session, utils
from flowie_runtime.data_types import (AgentCall, BuildOutput, ChangeCapture,
...
def main(prompt: str, config: str = ".flowie/flowie.config.yaml", adw_id: str | None = None) -> int:
    ...
    def commit(ph, envelope) -> None:
        ...
        message = envelope.commit_message or f"flowie({run.adw_id}): {envelope.summary}"
