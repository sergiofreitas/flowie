...
Usage:
    uv run adws/adw_plan_build_test.py "<prompt or path/to/prompt.md>" [--config .flowie/flowie.config.yaml] [--adw-id a1b2c3d4]

Phases: engineer(request) -> planner -> builder -> code(test) [-> builder(fix) -> code(test) ... bounded] -> git(commit)

Testing is CODE: the suite's command lives in .flowie/quality.py, so no
...
from flowie_runtime import agents, gates, git_helper, quality, session, utils
...
def main(prompt: str, config: str = ".flowie/flowie.config.yaml", adw_id: str | None = None) -> int:
    ...
    if test is not None and test.passed:
        with run.phase(PhaseParams(name="commit", kind="code", owner="git",
            ...
            message = previous.commit_message or f"flowie({run.adw_id}): {previous.summary}"
