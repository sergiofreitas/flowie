...
Usage:
    uv run adws/adw_document.py "<prompt or path/to/prompt.md>" [--base main] [--config .flowie/flowie.config.yaml] [--adw-id a1b2c3d4]
...
means here; see flowie_runtime/changes.py for how the base commit is resolved on a
...
from flowie_runtime import agents, changes, gates, session, utils
from flowie_runtime.data_types import (AgentCall, ChangeCapture, DocumentOutput,
                                    PhaseParams)
...
def main(prompt: str, base: str = "main",
         config: str = ".flowie/flowie.config.yaml", adw_id: str | None = None) -> int:
