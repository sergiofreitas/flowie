"""Concrete data types for the Flowie ADW system.
...
class AgentConfig(BaseModel):
    name: str
    coding_agent: Literal["codex", "pi", "claude_code"] = "codex"
    model: str = "gpt-5.5"
    thinking: str = "medium"        # mapped by the selected coding agent
    ...
    # (see flowie_runtime/permissions.py). `tools` cannot express this: `bash` runs
...
class ConfigDefaults(BaseModel):
    ...
    protected_files: list[str] = Field(default_factory=lambda: [
        ".flowie/flowie.config.yaml", ".flowie/adws/", ".flowie/quality.py",
    ])
    data_dir: str = ".flowie/data"


class ObservabilityConfig(BaseModel):
    db: str = ".flowie/data/flowie.db"
    poll_ms: int = 500


class FlowieConfig(BaseModel):
...
class CodexRequest(BaseModel):
    """Everything one non-interactive codex exec run needs."""
