...
def _project_func(name: str):
    """Return a project-local quality function when .flowie/quality.py defines it."""
    global _PROJECT_MODULE
    path = Path(".flowie/quality.py")
    if not path.exists():
        return None
    if _PROJECT_MODULE is None:
        spec = importlib.util.spec_from_file_location("flowie_project_quality", path)
        if spec is None or spec.loader is None:
            return None
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        _PROJECT_MODULE = module
    func = getattr(_PROJECT_MODULE, name, None)
    return func if callable(func) else None


def _placeholder(name: str) -> list[str]:
    """A command that does nothing and admits it. Replace every call to this."""
    return ["echo", f"PLACEHOLDER {name}: edit .flowie/quality.py and "
...
def typecheck(run) -> QualityCheckResult:
    if custom := _project_func("typecheck"):
        return custom(run)
    return _run(QualityCheckSpec(
        name="typecheck",
        area="backend",
...
def run_tests(run) -> QualityResult:
    ...
    """
    if custom := _project_func("run_tests"):
        return custom(run)
    check = test(run)
...
def run_quality(run) -> QualityResult:
    ...
    """
    if custom := _project_func("run_quality"):
        return custom(run)
