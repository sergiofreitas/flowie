/**
 * Flowie visualizer server — JSON API over a target repo's flowie.db, plus the
 ...
 *   bun run server/index.ts --db /path/to/repo/adws/adw_data/flowie.db
 *   FLOWIE_DB=/path/to/flowie.db PORT=4600 bun run server/index.ts
 */
import { existsSync, statSync } from "node:fs";
import { join, resolve, sep } from "node:path";
import { FlowieDb, resolveDbPath } from "./db.ts";
...
let db: FlowieDb;
try {
  db = new FlowieDb(dbPath);
} catch (error) {
  console.error(`[flowie] ${(error as Error).message}`);
  process.exit(1);
}
...
function safely(
  ...
  return async (req) => {
    try {
      ...
      console.error(`[flowie] ${req.method} ${new URL(req.url).pathname}:`, error);
...
async function serveStatic(req: Request): Promise<Response> {
  ...
  if (!existsSync(DIST_DIR)) {
    return new Response(
      `Flowie visualizer API is running on :${PORT}.\n\n` +
...
console.log(`[flowie] visualizer api  http://localhost:${server.port}`);
console.log(`[flowie] db              ${db.path}  [journal_mode=${db.journalMode}]`);
console.log(
  existsSync(DIST_DIR)
    ? `[flowie] serving ui from  ${DIST_DIR}`
    : `[flowie] no ./dist — use "bun run dev" for the Vite dev server on :4601`,
);
