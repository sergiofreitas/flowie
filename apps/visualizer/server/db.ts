/**
 * SQLite reader over a target repo's flowie.db.
 *
...
const DEFAULT_DB_RELATIVE = "adws/adw_data/flowie.db";
const MAX_LIMIT = 1000;
const DEFAULT_LIMIT = 500;

/**
 * Resolve the db path: --db arg wins, then FLOWIE_DB, then <cwd>/adws/adw_data/flowie.db.
...
export function resolveDbPath(argv: string[] = Bun.argv): string {
  ...
  const raw =
    ...
    process.env.FLOWIE_DB ??
    DEFAULT_DB_RELATIVE;

  return isAbsolute(raw) ? raw : resolve(process.cwd(), raw);
}

export class FlowieDb {
   ...
   * `adws/adw_data/flowie.db`), so deriving it as a sibling of the db file keeps
  ...
  constructor(path: string) {
    if (!existsSync(path)) {
      throw new Error(
        `flowie.db not found at ${path}\n` +
          `Point the visualizer at a target repo: --db <path> or FLOWIE_DB=<path>, ` +
